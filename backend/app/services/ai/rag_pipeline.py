...
class RAGPipeline:
    ...
    def search_relevant_chunks(self, query: str, doc_ids: list = None, n_results: int = 5) -> list:
        """
    ...
    def answer_user_query(self, query: str, doc_ids: list = None, history: list = None) -> dict:
        ...
        # Step 2: Query Classification Check (Bypass Gemini/RAG for generic greetings)
        ...
        # Step 3: Retrieve context from vector store (top 5 filtered chunks)
        chunks = self.search_relevant_chunks(query, doc_ids=doc_ids, n_results=5)
