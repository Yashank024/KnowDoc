...
def delete_chat(chat_id: str):
    """
    Erase a chat session thread from local persistence chats.json store and ChromaDB RAG QA memory.
    """
    logger.info(f"Deleting chat session thread: {chat_id}")
    
    # Delete RAG QA conversation cache/long-term memory associated with this chat
    try:
        rag_pipeline.delete_document_chunks(f"chat_memory_{chat_id}")
        logger.info(f"Successfully deleted long-term QA memory chunks for chat ID: {chat_id}")
    except Exception as e:
        logger.error(f"Failed to clear long-term QA memory chunks for chat {chat_id}: {e}")
        
    success = chat_service.delete_chat_session(chat_id)
    return {"status": "success", "message": f"Chat thread {chat_id} and its associated RAG memories deleted successfully."}
...
async def chat_direct(request: ChatRequest):
    ...
    # Auto-rename chat thread if it's the first message
    new_title = None
    try:
        new_title = chat_service.auto_rename_chat(request.chat_id, request.message)
    except Exception as e:
        logger.error(f"Failed to auto-rename chat: {e}")

    # 2. Run RAG Pipeline
    result = rag_pipeline.answer_user_query(request.message, doc_ids=request.doc_ids, history=request.history, chat_id=request.chat_id)
    
    # 3. Save Assistant Message
    chat_service.append_message_to_chat(request.chat_id, "ai", result.get("reply", ""), citations=result.get("citations"), session_id=request.session_id)
    
    if new_title:
        result["chat_title"] = new_title
