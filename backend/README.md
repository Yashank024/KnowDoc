...
│   │   │   └── chat.py          # Session-synced OpenRouter RAG chatbot router
│   │   └── deps/
...
│   │   └── prompts.py           # OpenRouter rigid markdown system prompt contract
│   ├── db/
...
│   ├── services/
│   │   ├── ai/
│   │   │   ├── openrouter_service.py # OpenRouter completions client wrapper
...
Configure your OpenRouter API key and model choice inside `backend/.env`:
```env
...
### 6. Dynamic RAG Prompt Contract & OpenRouter Response Post-Processing
...
*   **Technique**: Rigid System Prompt Contract & Backend Post-Processing.
    - Enforces a high-end system prompt contract instructing the OpenRouter model to strictly output rich markdown (hierarchical headings, tabular grids, bullet points) and index accurate visual inline bracket citations in the format `[Doc X, Page Y]`.
    - **Backend Post-Processor**: Prior to returning the final JSON reply, the backend parses the OpenRouter text, matches the matched citations, and strips out any unused document reference metadata. Only files actually cited in the text are returned inside the dynamic sources list.
...
*   **Technique**: Memory Window Constraint.
    - Implements a sliding window memory manager that dynamically structures only the **last 6 messages** as contextual memory for the OpenRouter conversational prompt, preserving natural dialogue flow while optimizing bandwidth.
...
*   **Technique**: On-Demand Singleton Model Loading.
    ...
    - CPU PaddleOCR, local SentenceTransformers, and OpenRouter generative configurations are completely lazy-loaded only when their respective action endpoints are first triggered by client uploads or chats.
